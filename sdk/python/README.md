...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `mysql` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `unobravo` with the `ORG` of your provider.
   - replace all instances of the `github.com/unobravo/pulumi-mysql` repository with the `REPOSITORY` location
...
├── config.yaml
├── metadata.json
├── pulumi-mysql_darwin_amd64_v1
│   └── pulumi-resource-mysql
├── pulumi-mysql_darwin_arm64
│   └── pulumi-resource-mysql
├── pulumi-mysql_linux_amd64_v1
│   └── pulumi-resource-mysql
├── pulumi-mysql_linux_arm64
│   └── pulumi-resource-mysql
├── pulumi-mysql_windows_amd64_v1
│   └── pulumi-resource-mysql.exe
└── pulumi-mysql_windows_arm64
    └── pulumi-resource-mysql.exe
```
